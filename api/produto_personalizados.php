...
add_action('rest_api_init', function () {
    register_rest_route('v1', '/produto', [
        'methods' => 'POST',
        'callback' => 'produto_personalizado_rest',
        'permission_callback' => '__return_true',
    ]);
});

function produto_personalizado_rest($request)
{
    // Obter todos os dados do corpo da requisição
    $body = json_decode($request->get_body(), true);
    
    // Se precisar enviar os dados para o RD Station
    $result = produto_personalizado_rd_station($body);

    return rest_ensure_response($result);
}

function produto_personalizado_rd_station($data)
{
